/* ============================================
   Me Offer 考研爬虫 · 单页 HTML 大数据解析
   适用于人大这类专业目录全在一页 HTML 里的情况
   Puppeteer 打开 → 拿文本 → 分块 Claude → 入 D1
   ============================================ */
...
const LOG_PATH					= os.homedir() + '/programme_scrapes/grad_html_' + new Date().toISOString().substring(0, 10) + '.log';
...
async function parse_all(uni_name, text) {
	...
	for (let i = 0; i < chunks.length; i++) {
		let r					= await claude_parse(uni_name, '第 ' + (i + 1), chunks[i]);
...
async function process_one(browser, target) {
	...
	try {
		...
		log_line('  err: ' + e.message.substring(0, 120));
		return 0;
	}
	finally {
...
async function main() {
	...
	let targets					= JSON.parse(fs.readFileSync('/root/meuni-apply-bot/html_urls2.json', 'utf8'));
	...
	for (let t of targets) {
		...
		grand					= grand + n;
		result.push({ name: t.name, count: n });
	}

	await browser.close();

	log_line('');
	result.forEach(function (r) { log_line('  ' + (r.count > 0 ? '✅' : '❌') + ' ' + r.name + ': ' + r.count); });
	log_line('🏁 总 ' + grand);
}
